...
def compare_brew_list(brew_list):
    ...
    for local_item in brew_list_local:
        if local_item in brew_list_remote_unknown:
            pass
        elif local_item not in brew_list_remote_name:
            local_item_dict = {}
    ...
    print('未安装:')
    diff_remote_with_local = []
    for remote_item in brew_list_remote_name:
        if remote_item not in brew_list_local:
            diff_remote_with_local.append(remote_item)
            for brew_item in brew_list:
                if brew_item['name'] == remote_item:
                    print(remote_item, 'tag:', brew_item['tag'], 'desc:', brew_item['desc'])
                    print('')
...
def compare_brew_cask_list(brew_cask_list):
    ...
    for brew_cask_item in brew_cask_list:
        brew_cask_list_remote_name.append(brew_cask_item['name'])

    diff_local_with_remote = []
    cask_item_list_new_install = []
    for local_item in brew_cask_list_local:
        if local_item not in brew_cask_list_remote_name:
            local_item_dict = {}
            local_item_dict['name'] = local_item
            local_item_dict['desc'] = ""
            local_item_dict['tag'] = ""
    ...
    print('未安装Cask:')
    diff_remote_with_local = []
    for remote_item in brew_cask_list_remote_name:
        if remote_item not in brew_cask_list_local:
            diff_remote_with_local.append(remote_item)
            for brew_cask_item in brew_cask_list:
                if brew_cask_item['name'] == remote_item:
                    print(remote_item, 'tag:', brew_cask_item['tag'], 'desc:', brew_cask_item['desc'])
                    print('')
...
if __name__ == '__main__':
    ...
    with open('brew_backup_2019122822.json') as backup_file:
        ...
        if is_select_update == 'Y':
            if len(tap_item_list_new_install) > 0 or len(item_list_new_install) > 0 or len(
                    cask_item_list_new_install) > 0:
